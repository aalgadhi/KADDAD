...
function TripCard({ ride, isArabic, userId, onComplete }) {
...
export default function Home() {
  ...
  const userId = (() => {
    try {
      return JSON.parse(localStorage.getItem('profileData'))?._id;
    } catch {
      return null;
    }
  })();
  ...
  const handleCompleteRide = async (tripId) => {
    ...
    try {
      ...
      if (res.ok && result.success) {
        // Remove the trip from both states
        setAll(prev => prev.filter(r => r._id !== tripId));
            ...
            <div className="row g-4">
              {filtered.length ? filtered.map((r) => (
                <div key={r._id} className="col-md-6 col-lg-4 d-flex">
                  <TripCard ride={r} isArabic={isArabic} userId={userId} onComplete={handleCompleteRide} />
                </div>
              )) : (
